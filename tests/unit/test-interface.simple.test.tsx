...
// Простые моки без TypeScript аннотаций
...
describe("TestInterface Component", () => {
  ...
  it("completes test when finish button is clicked", async () => {
    // Для этого теста используем тест с одним вопросом
    const singleQuestionTest = {
      ...mockTest,
      questions: [mockTest.questions[0]], // только один вопрос
    };

    render(
      <TestInterface
        test={singleQuestionTest}
        userId="user123"
        onComplete={mockOnComplete}
        onCancel={mockOnCancel}
      />
    ...
    // Finish the test - кнопка "Завершить тест" будет доступна, так как это последний вопрос
