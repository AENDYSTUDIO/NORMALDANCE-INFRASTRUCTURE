...
// Mock external dependencies for integration testing - using the same pattern as jest.setup.js
jest.mock("@solana/web3.js", () => {
  const originalModule = jest.requireActual("@solana/web3.js");
  return {
    ...originalModule,
    Connection: jest.fn().mockImplementation(() => ({
      getBalance: jest.fn().mockResolvedValue(2500000), // 2.5 SOL in lamports
    ...
    LAMPORTS_PER_SOL: 1000000000, // Standard value
  };
});
...
describe("NDT Staking Integration Tests", () => {
  ...
  beforeEach(() => {
    ...
    mockConnection = {
      getBalance: jest.fn().mockResolvedValue(2500000000), // 2.5 SOL in lamports
      getLatestBlockhash: jest.fn().mockResolvedValue({
        blockhash: "test-blockhash",
        lastValidBlockHeight: 10,
      }),
  ...
  describe("NDT Staking Business Logic", () => {
    ...
    it("should calculate staking rewards correctly", () => {
      ...
      expect(rewards).toBeCloseTo(12.33, 2); // Approximately 12.3 NDT
