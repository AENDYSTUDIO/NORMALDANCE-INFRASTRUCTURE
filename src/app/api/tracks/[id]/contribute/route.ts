...
export async function POST(
  ...
  try {
    ...
    if (track.status !== 'DRAFT') {
      return NextResponse.json(
        { error: 'Track is not in draft status' },
        { status: 400 }
      )
    }
    ...
    const result = await db.$transaction(async (tx) => {
      ...
      // Add contribution reward (10% bonus)
    ...
    return NextResponse.json({
      ...
      data: {
        newProgress: result.progressCurrent,
        isComplete: result.isProgressComplete,
        progressData
      }
    })
...
async function getProgressData(trackId: string) {
  const track = await db.track.findUnique({
    where: { id: trackId },
    select: {
      progressTarget: true,
